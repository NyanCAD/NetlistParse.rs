...
pub enum SyntaxKind {
    ...
    // --- Spectre-only form variants (forms.jl struct names) ---
    // Reused where a SPICE variant already matches by name+meaning: Parameter,
    // Model, Subckt, Parens, TernaryExpr, UnaryOp, BinaryExpression,
    // FunctionCall, FunctionArgs, Keyword, Operator, Identifier, Notation,
    // StringLiteral, NumberLiteral, Literal, BuiltinFunc, BuiltinConst,
    // Incomplete, Error, SystemIdentifier.
...
impl SyntaxKind {
    ...
    pub fn dump_label(self) -> &'static str {
        ...
        match self {
            ...
            SpectreNetlistSource => "SpectreNetlistSource",
            SNode => "SNode",
            SNodeList => "SNodeList",
            SubcktNode => "SubcktNode",
            SubcktNodes => "SubcktNodes",
            Instance => "Instance",
            Analysis => "Analysis",
            Save => "Save",
            SaveSignal => "SaveSignal",
            SaveSignalModifier => "SaveSignalModifier",
            Simulator => "Simulator",
            SpectreArray => "SpectreArray",
            Global => "Global",
            Ic => "Ic",
            ICParameter => "ICParameter",
            NodeSet => "NodeSet",
            Alter => "Alter",
