...
impl Lexer {
    ...
    /// keyed to the original source.
    pub fn new(src: &str, spice_dialect: Dialect, strict: bool, enable_julia_escape: bool) -> Self {
    ...
    pub fn tokenize(src: &str, dialect: Dialect, strict: bool, enable_julia_escape: bool) -> Vec<RawTok> {
        let mut lx = Lexer::new(src, dialect, strict, enable_julia_escape);
        let mut out = Vec::new();
        loop {
            let t = lx.next_token();
